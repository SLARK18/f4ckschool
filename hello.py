...
@application.route('/')
def hello():
    return render_template("base_text_info.html", html_stat=open(r'templates/История_развития/Первые_компьютерные_сети.html', encoding='utf-8').read(), menu_list=MENU_LIST, left_menu={})

# @application.route('/proverka')
# def rtest():
#     return render_template("base_text_info.html", html_stat=open(r'templates/История_развития/Первые_компьютерные_сети.html', encoding='utf-8').read(), menu_list=MENU_LIST)
